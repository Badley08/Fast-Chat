...
import { 
    ...
    createGroup, searchUsers, joinGroupByName, setTypingStatus, listenToTyping, getOrCreatePrivateChat
} from './ChatService.js';
...
$('anonForm').addEventListener('submit', async (e) => {
    ...
    const icon = $('anonIcons').dataset.selected || 'user-secret';
    try {
        await loginAnonymously(name, color, icon);
    } catch (err) { showToast(err.message, 'error'); }
});

$('phoneForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const phonePrefix = $('phonePrefix').value || '+1';
    const phone = phonePrefix + $('phoneInput').value;
    try {
...
$('userSearchInput').addEventListener('input', async (e) => {
    ...
    users.forEach(u => {
        ...
        div.style.gap = '15px';
        div.innerHTML = `
            <div class="chat-avatar" style="background:${u.color || '#1EA7FF'}"><i class="fa-solid fa-${u.icon || 'user'}"></i></div>
            <div>${u.username}</div>
        `;
        div.onclick = async () => {
            try {
                const chat = await getOrCreatePrivateChat(u);
                openConversation(chat.id, u.username, 'private');
...
function setupIconSwatches() {
    const icons = ['user', 'user-ninja', 'cat', 'dog', 'bolt', 'star', 'fire', 'heart', 'gem', 'music'];
    const targets = ['anonIcons', 'settingsIcons'];
    ...
    targets.forEach(t => {
        ...
        icons.forEach(i => {
            ...
            div.innerHTML = `<i class="fa-solid fa-${i}"></i>`;
            div.onclick = () => {
                el.querySelectorAll('.icon-swatch').forEach(s => s.classList.remove('active'));
                div.classList.add('active');
